...
class ChurchScreenState extends State<ChurchScreen> {
  @override
  void initState() {
    getChurch();
    super.initState();
  ...
  void _searchChurch(String church) {
    ...
    final newChurchs = churchsCopy.where(
        (element) => element.nom.toLowerCase().contains(church.toLowerCase()));
    setState(() {
      churchs = newChurchs.toList();
    });
  }

  void getChurch({bool order = false}) async {
    setState(() {
      gettingChurch = true;
    });
                  ...
                  children: <Widget>[
                    ...
                    RichText(
                      ...
                      text: TextSpan(
                          ...
                          children: <TextSpan>[
                            TextSpan(
                                text: church.description.length >= 60
                                    ? '${church.description.substring(0, 60).toLowerCase()}...'
                                    : church.description.toLowerCase(),
                                style: const TextStyle(color: Colors.black)),
                            if (church.description.length >= 60)
                              const TextSpan(
                                  text: ' Voir plus',
                                  style: TextStyle(
                          ...
                          children: <Widget>[
                            isShowSearchField == true
                                ? InputField(
                                    ...
                                    labelText: "Rechercher une église",
                                    label: "",
